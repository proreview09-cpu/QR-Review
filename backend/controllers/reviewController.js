...
exports.copyReview = async (req, res) => {
  try {
    ...
    if (unusedCount < shop.reviewPoolMin) {
       generateReviews(shop.shopName, shop.businessName, shop.reviewTone, shop.reviewBatchSize || 10, shop.language, shop._id, shop.aiPrompt || '')
        .then(async (reviews) => {
...
exports.generateMore = async (req, res) => {
  try {
    ...
    if (needed > 0) {
       const reviews = await generateReviews(shop.shopName, shop.businessName, shop.reviewTone, needed, shop.language, shop._id, shop.aiPrompt || '');
      await Review.insertMany(reviews.map((content) => ({ shop: shop._id, content })));
